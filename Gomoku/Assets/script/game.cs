	...
	void Update () {
		...
		else if (gameMode == 0 && playerJustPlayed)
		{
			...
			for (int l = 0; l < toDo; l++){
				temp = toplay[l];
				if (turn == 1)
					temp.z = AlphaBeta (goToIntWithLocal(currentGame, (int)toplay[l].x, (int)toplay[l].y, 1), 3, int.MinValue, int.MaxValue);//H.GetCostFromMapStatus(goToIntWithLocal((int)Mathf.Round(toplay[l].x * 2), (int)Mathf.Round(toplay[l].y * -2), 0), 1, 0, Captured_by_white, Captured_by_black);//AlphaBeta (toplay[l], 4, -Mathf.Infinity, Mathf.Infinity);
				else
